    ...
    def forward(self, x):
        if self.training:
            ...
            for i in range(self.nl):
                ...
                cls_output = torch.sigmoid(self.cls_preds[i](cls_feat))
                #
                reg_feat = self.reg_convs[i](reg_x)
                reg_output = self.reg_preds[i](reg_feat)
                #
                cls_score_list.append(cls_output.flatten(start_dim=2, end_dim=3).permute((0, 2, 1)))
                reg_distri_list.append(reg_output.flatten(start_dim=2, end_dim=3).permute((0, 2, 1)))
            ...
            reg_ldmk_list = torch.cat(reg_ldmk_list, dim=-1).permute(0, 2, 1)
