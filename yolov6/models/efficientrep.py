...
class EfficientRep(nn.Module):
    ...
    def __init__(self,
                 in_channels=3, channels_list=None,
                 num_repeats=None, block=RepVGGBlock,
                 fuse_P2=False, cspsppf=False, use_se=False, attention='CBAM'):
        super().__init__()
        ...
        # See configs/base/yolo6l_base.py, which is one of network configuration files.
        # The file includes network configuration such as out_channels (channels_list), num_repeats, and etc.
        self.stem = block(
    ...
    def forward(self, x):
        outputs = []
        x = self.stem(x)  # P1
        x = self.ERBlock_2(x)  # P2
        if self.fuse_P2:
            outputs.append(x)
        x = self.ERBlock_3(x)  # P3
        outputs.append(x)
        x = self.ERBlock_4(x)  # P4
        outputs.append(x)
        x = self.ERBlock_5(x)  # P5
        outputs.append(x)
