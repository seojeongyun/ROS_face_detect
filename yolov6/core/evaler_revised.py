...
'''
python tools/core_eval.py --task 'train'/'val'/'speed'
'''
...
class Evaler:
    ...
    def init_model(self, model, weights, task):
        if task == 'train':
            # Evaluation can be conducted in training
            pass
        else:
            # Load weights
            if not os.path.exists(weights):
                LOGGER.error(f'There is no weight {weights}')
                raise
            #
            ...
            # Switch to deploy mode of RepVGGBlock
        ...
        # half() means float16() while float() is float32().
    ...
    def predict_model(self, model, dataloader, task):
        ...
        for i, (ori_img, imgs, targets, paths, shapes) in enumerate(pbar):
            ...
            self.speed_result[1] += time.time() - t1
            # Inference
            #t2 = time_sync()
            t2 = time.time()
            outputs, _ = model(imgs)
            self.speed_result[2] += time.time() - t2  # inference time

            # post-process
            # t3 = time_sync()
            t3 = time.time()
            outputs = non_max_suppression_face(outputs, self.conf_thres, self.iou_thres, multi_label=True)
            self.speed_result[3] += time.time() - t3  # post-process time
            ...
            if len(vis_outputs) < 20:
                for out_i in range(len(outputs)):
                    if len(outputs[out_i]) > 0:
                        vis_outputs.append(outputs[out_i])
                        vis_paths.append(paths[out_i])
...
if __name__ == '__main__':
    anno_json = '/storage/hrlee/widerface/annotations/instances_val.json'
    pred_json = '/home/hrlee/PycharmProjects/YOLOv6/runs/train/6l_hs2/predictions.json'
    ...
    LOGGER.info(f"mAP@0.5: {map} | mAP@0.50:0.95: {map50}")
