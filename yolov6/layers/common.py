...
class RepVGGBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
                 stride=1, padding=1, dilation=1, groups=1, padding_mode='zeros', deploy=False, use_se=False, attention='CBAM'):
        ...
        if use_se:
            self.reduction = 4 if out_channels <= 128 else 8
            if attention == 'channel':
                self.se = ChannelAttention(in_channels=out_channels, reduction=self.reduction)
            elif attention == 'spatial':
                self.se = SpatialAttention(in_channels=out_channels, reduction=self.reduction)
            elif attention == 'CBAM':
                self.se = CBAM(in_channels=out_channels, reduction=self.reduction)
            else:
                raise NotImplementedError('{} is not implemented.'.format(attention))
...
class LinearAddBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1,
                 dilation=1, groups=1, padding_mode='zeros', use_se=False, is_csla=False, conv_scale_init=1.0, attention='CBAM'):
...
class CBAM(nn.Module):
    def __init__(self, in_channels, reduction=4):
        ...
        self.conv_se = nn.Conv2d(in_channels=in_channels, out_channels=in_channels, kernel_size=3, stride=1, padding=1)
        self.conv = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=7, stride=1, padding=3)
        self.relu = nn.ReLU()

    def forward(self, inputs):
        F_ = self.conv_se(inputs)
        # Channel Attention Module
        maxpool = F.adaptive_max_pool2d(F_, (1, 1))
        avgpool = F.adaptive_avg_pool2d(F_, (1, 1))
        Mc = torch.sigmoid(self.fc2(self.relu(self.fc1(maxpool.flatten(start_dim=1, end_dim=3))))
                           + self.fc2(self.relu(self.fc1(avgpool.flatten(start_dim=1, end_dim=3)))))
        Mc = Mc.reshape(inputs.shape[0], self.in_channels, 1, 1)
        # F'
        F_prime = F_ * Mc

        # Spatial Attention Module
        maxpool = torch.max(F_prime, dim=1)[0]
        avgpool = torch.mean(F_prime, dim=1)
        Ms = torch.sigmoid(self.conv(torch.stack((maxpool, avgpool), dim=1)))
        # F''
        F_two_prime = F_prime * Ms
        # output
        out = F_two_prime + inputs
        return out
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # Spatial Attention Module
        maxpool = torch.max(F_, dim=1)[0]
        avgpool = torch.mean(F_, dim=1)
        Ms = torch.sigmoid(self.conv(torch.stack((maxpool, avgpool), dim=1)))
        # F''
        F_two_prime = F_ * Ms
        # output
...
class RepBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n=1, block=RepVGGBlock, basic_block=RepVGGBlock, use_se=False, attention='CBAM'):
        super().__init__()

        self.conv1 = block(in_channels, out_channels, use_se=use_se, attention=attention)
...
class BottleRep(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, basic_block=RepVGGBlock, weight=False, use_se=False, attention='CBAM'):
        super().__init__()
...
class MBLABlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n=1, e=0.5, block=RepVGGBlock, use_se=False):
        super().__init__()
        n = n // 2
        if n <= 0:
            n = 1
...
if __name__ == '__main__':
    ...
    # For debugging of the training mode
    repvggblock = RepVGGBlock(in_channels=3, out_channels=64,
                              kernel_size=3, stride=1, padding=1,
                              dilation=1, deploy=False)
    #
    opt = torch.optim.Adam(repvggblock.parameters(), lr=1e-5, betas=(0.9, 0.999))

    #
    test_input = torch.rand([3, 3, 25, 25], requires_grad=True)
    test_gt = torch.ones([3, 3, 25, 25])

    out = repvggblock(test_input)
    loss = F.mse_loss(test_input, test_gt)

    opt.zero_grad()
    loss.backward()
    opt.step()

    # For debugging of the deploy mode
    for layer in repvggblock.modules():
        if isinstance(layer, RepVGGBlock):
            layer.switch_to_deploy()
    out_deploy = repvggblock(test_input)
