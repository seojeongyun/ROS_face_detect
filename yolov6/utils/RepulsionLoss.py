...
def repulsion_loss(pbox, gtbox, fg_mask, sigma_repgt=0.9, sigma_repbox=0, pnms=0, gtnms=0, device=torch.device("cpu")):  # nms=0
    ...
    for idx in range(pbox.shape[0]):
        ...
        pgiou = pgiou.to(pbox.device).data.cpu().numpy()
        ppiou = ppiou.to(pbox.device).data.cpu().numpy()
        _gtbox_pos_cpu = _gtbox_pos.to(pbox.device).data.cpu().numpy()
